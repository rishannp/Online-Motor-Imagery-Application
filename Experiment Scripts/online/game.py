...
from config import (
    SESSION_DIR, NUM_LEVELS, TRIALS_PER_LEVEL,
    CUE_DURATION, TRIAL_DURATION,
    INTER_TRIAL_PAUSE, INTER_LEVEL_PAUSE,
    SAMPLING_RATE, PLAYER_SPEED
)
...
def run_game(action_q, eeg_chunk_q):
    """
    Saves per trial:
      - eeg: [n_ch, n_samples] continuous
      - label: 0/1 target side
      - hit: bool
      - cursor_x: per-frame x trace
    """
    pygame.init()
    screen = pygame.display.set_mode((SCREEN_W, SCREEN_H))
    pygame.display.set_caption("Lane Runner (GAT Online)")
    ...
    STATE_TRIAL, STATE_ITI, STATE_ILP = 0, 1, 2
    state = STATE_TRIAL
    ...
    while run and level <= NUM_LEVELS:
        ...
        if state == STATE_TRIAL:
            ...
            # AR-like accumulation: constant per-frame step, driven by latest cmd
            ...
            # drain EEG samples for continuous recording
            while True:
                try:
                    chunk = eeg_chunk_q.get_nowait()   # [1, n_ch]
            ...
            screen.blit(font.render(f"Next target: {next_txt}", True, TEXT_COLOR), (DOT_CENTER_X - 110, 60))
            screen.blit(font.render(f"{remain_ms/1000:.1f}s", True, TEXT_COLOR), (DOT_CENTER_X - 20, 90))
            ...
            next_txt  = "LEFT" if side == 0 else "RIGHT"
            screen.blit(font.render("Level complete — Rest", True, TEXT_COLOR), (DOT_CENTER_X - 170, 20))
            screen.blit(font.render(f"Next target: {next_txt}", True, TEXT_COLOR), (DOT_CENTER_X - 110, 60))
            screen.blit(font.render(f"{remain_ms/1000:.1f}s", True, TEXT_COLOR), (DOT_CENTER_X - 20, 90))
